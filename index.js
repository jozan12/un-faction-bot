...
client.once("ready", async () => {
  ...
  const commands = [
    ...
    new SlashCommandBuilder()
      ...
      .setDescription("View all members of a faction")
      .addStringOption(o => o.setName("name").setDescription("Faction name").setRequired(true)),
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.commandName === "faction-members") {
      ...
      db.all("SELECT user_id FROM users WHERE faction = ?", [name], async (err, rows) => {
        ...
        let memberList = await Promise.all(
          rows.map(async r => {
            try {
              ...
              return `• <@${r.user_id}> (left server)`;
            }
          })
        );

        interaction.reply(
          `**👥 Members of ${name} (${rows.length})**\n\n${memberList.join("\n")}`
        );
      });
    }

    // (ALL YOUR OTHER COMMANDS REMAIN UNCHANGED BELOW)
    // ——————————————
